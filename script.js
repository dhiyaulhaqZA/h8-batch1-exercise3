...
function showVehicles(vehicles) {
    var vehicleTableHtml = "";
    for (var i = 0; i < vehicles.length; i++) {
        var vehicle = vehicles[i]
        vehicleTableHtml+="<tr>";
        vehicleTableHtml+="<th scope=\"row\">" + (i+1) + "</th>"
        vehicleTableHtml+="<td>" + vehicle.name + "</td>";
        vehicleTableHtml+="<td>" + vehicle.model + "</td>";
        vehicleTableHtml+="<td>" + vehicle.manufacturer + "</td>";
        vehicleTableHtml+="<td>" + vehicle.length + "</td>";
        vehicleTableHtml+="<td>" + vehicle.max_atmosphering_speed + "</td>";
        vehicleTableHtml+="</tr>";
    }

    document.getElementById("vehicle-table").innerHTML = vehicleTableHtml;
}

showVehicles(vehicles.results)

function myFunction() {
    var input = document.getElementById("search-vehicle-input").value;
    showVehicles(filterVehicles(input))


    var vehicleTable = document.getElementById("vehicle-table");
    vehicleTable.removeChild;
}

function filterVehicles(name) {
    return vehicles.results.filter(it => it.name.includes(name))
}